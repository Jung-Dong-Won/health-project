...
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"Args":["Health_register","N00001","Kim"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
